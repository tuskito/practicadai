...
function leerDatos_Compania(){
	...
	if (oXML.readyState == 4) {
		...
		for (i = 0; i < xml.getElementsByTagName('compania').length; i++){
		  ...
		  document.getElementById("form_compania").action = "compania.asp?compania="+ compania

		  definicion_tabla = definicion_tabla + "<tr><td colspan='4'><a href='compania.asp?compania=" + compania + "&eliminar=true'><button form='#'>Eliminar</button></a></td></tr>";
		}
...
function mostrar_companias(idCompania){  
  ...
  oXML.open('GET', 'consulta.asp?compania=' + idCompania);
