							...
							if (idVuelo<>"") then
								...
								if (rs_vuelo("N_PLAZAS_DISPONIBLES") >= plazas) then 
									if (Request.QueryString("vuelta")<>"") then
										Response.write("<form action='reserva_res.asp?idvuelo=" & rs_vuelo("IDVUELO") & "&vuelta=true' method='post'>")
									else
										response.write("<form action='reserva_res.asp?idvuelo=" & idVuelo & "' method='post'>")
									end if
