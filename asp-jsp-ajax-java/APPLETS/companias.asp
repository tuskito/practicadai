					...
					<div class="Quienes.somos">
						...
						<%
							...
							if (request.form("1")<>"") then
								...
								response.write("<tr><td colspan='2'>Se han modificado correctamente los datos de la compania</td></tr>")
							end if
							
							Set rs_compania=Conexion.Execute("select * from COMPANIA where " & compania & "=IDCOMPANIA")

						%>
							<tr><td>IdCompania</td><td>Compania</td></tr>
							<%
								Do Until rs_compania.EOF
									response.write("<form action='avion.asp?compania=" & compania & "' method='post'><tr><td>" & rs_compania("IDCOMPANIA") & "</td>")
	    							for i = 1 to rs_compania.fields.Count - 1 
	    								response.write("<td>" & "<input type='text' value='" & rs_compania(i) & "' name='" & i & "'/>" & "</td>")
	    							next
	    							response.write("<tr><td colspan='2'><input type='submit' value='Modificar'/></td></tr>")
	    							
	    							response.write("</tr></form>")
									rs_compania.MoveNext

								Loop
								response.write("<tr><td colspan='2'><a href='compania.asp?compania=" & compania & "&eliminar=true'><button class='boton_form'>Eliminar</button></a></td></tr>")
							%>
						</table>
					</div>
			</section>
