							...
							if (((usuario = "admin") AND (password="admin")) OR (session("login")=true)) then
								...
								if (Request.querystring("anadir")="true") then	
									...
									Do Until rs_compania.EOF
										%><tr><td><% 
										response.write("<a onclick='return mostrar_companias("& rs_compania("IDCOMPANIA") &")' href='#'>")
										response.write(rs_compania("COMPANIA"))
										response.write("</a>")
										%></td></tr><%
										rs_compania.MoveNext
									Loop
									response.write("<tr><td><a href='login.asp?anadir=true'><input type='submit' value='Añadir'/></td></tr>")
								end if
								%>
									</table>
									<form id='form_compania' action='#' method='post'>
										<table id="tabla_compania" class="tabla">
										</table>
									</form>
								<%
							else
